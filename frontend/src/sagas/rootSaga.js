...
export function* fetchLatestPairingList() {
    try {
        ...
        let pairingListId = response.data.internalId;
        let pairingList = response.data.pairingList;
        yield put(storePairingListId(pairingListId ? pairingListId : ""));
        yield put(storePairingList(pairingList ? pairingList : []));
    } catch (error) {}
}
...
export function* deleteHuman(action) {
    try {
        yield call(apiPut, '/humans/remove', action.human);
        yield call(fetchHumans);
    } catch (error) {}
}
